...
def checkLogin():
    ...
    if len(data):
        ...
        response = app.response_class(response=json.dumps({"message": "something went wrong please try again"}), status=400, mimetype='application/json')
    return response
...
def checkSignUp():
    ...
    if data:
        ...
        response = app.response_class(response=json.dumps({"message" :"Something Went Wrong please try again later"}), status=400, mimetype='application/json')
    return response
...
def postFoodData():
    ...
    if data:
		#valid User
        response = app.response_class(response=json.dumps({"message" : "Your request has been submitted"}), status=200, mimetype='application/json')
    else:
		#invalid user since we cannot find any one with matching name
        response = app.response_class(response=json.dumps({"message" : "Something went wrong while processing you request"}), status=400, mimetype='application/json')
    return response
...
def forgetPassword():
    ...
    if len(data):
        ...
        if limitCount > 5:
            response = app.response_class(response=json.dumps({"message": "Retry Limit exceeded"}), status=400, mimetype='application/json')
            return response
        elif limitCount:
...
def resetPassword():
    ...
    if redisValue != user_id:
        response = app.response_class(response=json.dumps({"message": "Invalid User"}), status=401, mimetype='application/json')
        return response
